    ...
    <main>
        <div class="container">
            ...
            <div class="main-contents">
                ...
                $uploadFilePath = $uploadDir . basename($upimg);
                if (move_uploaded_file($_FILES['upimg']['tmp_name'], $uploadFilePath)) {
                    // データをカンマ区切りの文字列にして保存
                    $str = $name . $br . $date . $br . $sex . $br . $breed . $br . $surgery . $br . $chip . $br . $record_date . $br . $record_place . $br . $record_number . $br . $uploadFilePath;
                ...
                // fopenで変数に格納したファイルを読み込む（"r"は読み込みモード）
                $fp = fopen($filename, "r");
...
<th>写真</th>
                ...
                while (($line = fgetcsv($fp)) !== FALSE) {
                    ...
                    for ($i = 0; $i < count($line); $i++) {
                        if ($i == 9) {
                            echo "<td><img src='" . $line[$i] . "' alt='Image' style='width:100px; height:auto;'></td>";
                        } else {
                            echo "<td>" . $line[$i] . "</td>";
                        }
                    }
